...
#
#     def forward(self, x):
#         b, l, c = x.shape  # x shape is (batch_size, sequence_length, hidden_dim)
#         # logits = self.mlp_head(x[:, 1:, :])  # logits shape is (batch_size, sequence_length-1, num_sub_layer)
#         logits = self.mlp_head(x)  # logits shape is (batch_size, sequence_length-1, num_sub_layer)
#
#         # Ensure that token_select has the correct last dimension
#         token_select = _gumbel_sigmoid(logits, self.tau, self.is_hard, threshold=self.threshold, training=self.training)
#         # token_select = torch.cat([torch.ones(b, 1, token_select.shape[-1], device=token_select.device), token_select],
#         #                          dim=1)
#
...
class TokenSelect(nn.Module):
    ...
    def forward(self, x):
        logits = self.mlp_head(x)  # logits shape is (batch_size, sequence_length, num_sub_layer)
