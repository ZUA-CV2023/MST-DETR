...
from ..modules.transformer import TselfATTENTION
...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'DynamicInterpolationFusion', 'FuseBlockMulti', 'C2f_FMB', 'gConvC3', 'C2f_gConv', 'LDConv',
           'C2f_TselfATTENTION']
...
############################################3
class C2f_TselfATTENTION(nn.Module):
    def __init__(self, c1, c2, n_blocks=1, ffn_scale=2.0, shortcut=False, g=1, e=0.5, drop_prob=0.2):
        super().__init__()

        # C2f部分
        self.c2f = C2f(c1, c2, n=n_blocks, shortcut=shortcut, g=g, e=e)

        # TselfATTENTION部分
        self.attention = TselfATTENTION(c2, c2)

    def forward(self, x):
        # 先通过 C2f 模块提取特征
        x_c2f = self.c2f(x)

        # 然后通过 TselfATTENTION 进一步加强特征
        x_attention = self.attention(x_c2f)
